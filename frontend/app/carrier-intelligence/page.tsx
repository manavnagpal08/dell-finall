...
import {
  ...
  ChevronDown, Network, Box, TrendingUp, Star, Award, Clock
} from "lucide-react"
    ...
    <div className="flex min-h-screen bg-[#FDFDFD] font-sans text-slate-800">
      ...
      <div className="flex-1 flex flex-col min-w-0 max-w-[1400px] mx-auto pb-10 relative">
        ...
        <div className="px-8 flex-1 space-y-6">
                      ...
                      <div className="w-[180px] shrink-0 flex flex-col items-end justify-center">
                        <div className="flex items-start gap-4 w-full">
                           <div className="flex-1 text-center pr-2">
                             ...
                             <div className={`inline-flex px-2 py-0.5 rounded border text-[10px] font-bold uppercase tracking-wider ${badge.color}`}>
                               {badge.label}
                             </div>
                           </div>
                           <div className="flex flex-col items-center justify-center pt-3">
                             <button className="px-4 py-2 rounded-lg border border-slate-200 text-[11px] font-bold text-slate-600 flex items-center gap-1.5 hover:bg-slate-50 transition-colors shadow-sm">
                            ...
                            <tr key={row.id} className={`transition-all duration-300 group ${isMatch ? "bg-emerald-50/30 hover:bg-emerald-50 border-l-[3px] border-l-[#00B67A] hover:shadow-[inset_0_0_15px_rgba(16,185,129,0.05)]" : "hover:bg-slate-50/50 border-l-[3px] border-l-transparent"}`}>
                              ...
                              <td className="p-5">
                                <div className="flex items-center gap-4">
                                  ...
                                  <button className="px-3 py-1.5 rounded-full border border-slate-200 text-[10px] font-bold text-slate-600 flex items-center gap-1 hover:bg-slate-50 group-hover:border-slate-300 transition-colors">
          ...
          {activeTab === "Simulator" && (
            <section className="rounded-3xl border border-emerald-100 bg-white p-8 shadow-sm animate-in fade-in zoom-in-95 duration-300">
              <div className="grid gap-8 lg:grid-cols-[1fr_340px] lg:items-center">
                <div>
                  <h2 className="text-2xl font-black text-slate-900 flex items-center gap-3"><Zap className="w-6 h-6 text-amber-500"/> Carrier Switch Simulator</h2>
                  <p className="mt-3 text-sm font-semibold leading-relaxed text-slate-500">
                    Uses the current lane matrix and carrier cost/SLA scorecard to estimate the effect of moving eligible lanes to the best carrier.
                  </p>
                  <div className="mt-8 grid gap-4 md:grid-cols-3">
                    <div className="rounded-2xl border border-slate-100 bg-slate-50 p-5">
                      <p className="text-[10px] font-black uppercase tracking-widest text-slate-400">Eligible Lanes</p>
                      <p className="mt-2 text-2xl font-black text-slate-900">{laneMatrix.filter((row) => row.savings > 0).length}</p>
                    </div>
                    <div className="rounded-2xl border border-slate-100 bg-slate-50 p-5">
                      <p className="text-[10px] font-black uppercase tracking-widest text-slate-400">Projected Saving</p>
                      <p className="mt-2 text-2xl font-black text-emerald-600">{money(laneMatrix.reduce((sum, row) => sum + row.savings, 0))}</p>
                    </div>
                    <div className="rounded-2xl border border-slate-100 bg-slate-50 p-5">
                      <p className="text-[10px] font-black uppercase tracking-widest text-slate-400">Target Carrier</p>
                      <p className="mt-2 text-xl font-black text-slate-900 truncate">{bestCarrier.name}</p>
                    </div>
                  </div>
                  <button onClick={() => setScenarioReady(true)} className="mt-8 rounded-xl bg-amber-500 px-6 py-3 text-xs font-black text-white shadow-sm hover:bg-amber-600 hover:shadow-md transition-all flex items-center gap-2">
                    <Zap className="w-4 h-4 fill-current"/> Run Simulation Engine
                  </button>
                </div>
                <div className={`rounded-3xl border p-6 min-h-[300px] flex flex-col justify-center transition-colors duration-500 ${scenarioReady ? "border-emerald-200 bg-emerald-50/50" : "border-slate-100 bg-slate-50/50"}`}>
                  {scenarioReady ? (
                    <div className="animate-in fade-in zoom-in duration-500">
                      ...
                      <h3 className="text-center font-black text-emerald-900 text-lg mb-2">Simulation Complete</h3>
                      <p className="text-xs font-bold leading-relaxed text-emerald-700/80 text-center">
                        Switching eligible lanes to <strong className="text-emerald-900">{bestCarrier?.name}</strong> can recover <strong className="text-emerald-900">{money(laneMatrix.reduce((sum, row) => sum + row.savings, 0))}</strong> while preserving carrier SLA discipline.
                      </p>
                    </div>
                  ) : (
                    <div className="text-center text-slate-500">
                      <AlertTriangle className="mx-auto mb-4 h-10 w-10 text-slate-300" />
                      <p className="text-xs font-bold">Run the simulator to calculate impact from real lanes.</p>
                    </div>
                  )}
                </div>
              </div>
