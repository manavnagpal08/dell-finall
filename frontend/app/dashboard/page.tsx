...
export default function MissionControlPage() {
  ...
  return (
    <div className="min-h-screen bg-slate-50/30 pb-24 font-sans text-[#0F172A]">
      <div className="mx-auto max-w-[1600px] space-y-6 p-6 lg:p-8">
        <motion.div initial="hidden" animate="visible" variants={fadeUp} className="mb-8 flex flex-col items-start justify-between md:flex-row md:items-center">
          <div>
            ...
            <p className="mt-1 text-[13px] font-medium text-slate-500">Real-time AI intelligence across your logistics network.</p>
          </div>
          <div className="mt-4 flex items-center gap-4 md:mt-0">
            <div className="flex items-center gap-4 rounded-full border border-slate-200 bg-white px-4 py-2 shadow-sm">
              <div className="flex items-center gap-2">
                <span className={`h-2 w-2 rounded-full ${backendError ? "bg-rose-500" : "bg-[#00B67A]"}`} />
                <span className="text-[13px] font-bold text-[#0F172A]">{backendError ? "API Issue" : "Live"}</span>
              </div>
              <div className="h-4 w-px bg-slate-200" />
              <button onClick={handleRefresh} className="flex items-center gap-2 text-[13px] font-medium text-slate-500 transition-colors hover:text-emerald-700">
                <RefreshCw className={`h-3.5 w-3.5 ${isDashboardLoading ? "animate-spin" : ""}`} /> Refresh data
              </button>
            </div>
            <div className="relative rounded-full border border-slate-200 bg-white p-2.5 text-slate-600 shadow-sm">
              <Bell className="h-4 w-4" />
              {tamperAlertPct > 0 && <div className="absolute right-0 top-0 h-2 w-2 rounded-full bg-rose-500" />}
            </div>
            <div className="flex h-9 w-9 items-center justify-center rounded-full border border-slate-200 bg-slate-100 text-[13px] font-bold text-slate-600 shadow-sm">
              SB
            </div>
          </div>
        </motion.div>
      ...
      <div className="fixed bottom-0 left-0 right-0 z-40 flex items-center justify-between border-t border-slate-200 bg-white p-3 px-8">
